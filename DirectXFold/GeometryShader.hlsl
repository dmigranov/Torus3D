...
struct VertexOutput
{
    float4 color : COLOR;
	//noperspective 
		float3 edgeDistance : EDGEDISTANCE;
	float fogFactor : FOG_FACTOR;
	float2 tex : TEXCOORD0;
...
void SimpleGeometryShader(triangle VertexInput input[3], inout TriangleStream<VertexOutput> stream)
{
	...
	VertexOutput v1 = { /*input[0].*/color, float3(ha, 0, 0), input[0].fogFactor, input[0].tex, input[0].position };
	stream.Append(v1); // добавление вертекса
	VertexOutput v2 = { /*input[1].*/color, float3(0, hb, 0), input[1].fogFactor, input[1].tex, input[1].position };
	stream.Append(v2); // добавление вертекса
	VertexOutput v3 = { /*input[2].*/color, float3(0, 0, hc), input[2].fogFactor, input[2].tex, input[2].position };
